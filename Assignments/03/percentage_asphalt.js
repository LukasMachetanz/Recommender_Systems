      ...
      setTimeout(() => {
          queryOverpass(buildQuery(gpsData[gps]), (error, data) => {
              if (error || data.features[0] === undefined) {
                  ...
                  resolve({
                      id : gps,
                      surfaceType : data.features[0].properties.tags.surface,
...
Promise.all(promises)
    .then(values => {
      ...
      for(let value in values) {
        ...
        (distribution[values[value].surfaceType] !== undefined) ? distribution[values[value].surfaceType] += dist : distribution[values[value].surfaceType] = dist;
